window.eventManager = (() => {
    ...
    const debouncedUpdateInsightsInput = debounce(() => {
    ...
    function handleBodyChange(event) {
        ...
        const changeActions = {
            ...
            'insights-comparison-select': () => handleInsightsStudyChange(target.value),
        ...
        if (target.name === 'comparisonView' || target.name === 'effectSizeType' || target.name === 'insights-feature-importance-method') {
    ...
    function handleBodyInput(event) {
        ...
        } else if (target.id.startsWith('insights-')) {
            handleInsightsInputChange(target);
        }
    }
    ...
    function handleInsightsStudyChange(studyId) {
        if (window.state.setInsightsStudyId(studyId)) {
            app.refreshCurrentTab();
        }
    }
    ...
    function handleRadioChange(radio) {
        let changed = false;
        switch (radio.name) {
            case 'comparisonView': changed = window.state.setComparisonView(radio.value); break;
            case 'effectSizeType': changed = window.state.setInsightsEffectSizeType(radio.value); break;
            case 'insights-feature-importance-method': changed = window.state.setInsightsFeatureImportanceMethod(radio.value); break;
        }
        if (changed) {
            app.refreshCurrentTab();
        }
    ...
    function handleInsightsInputChange(input) {
        let changed = false;
        const value = parseFloat(input.value);
        switch (input.id) {
            case 'insights-power':
                changed = window.state.setInsightsPower(value);
                break;
            case 'insights-alpha':
                changed = window.state.setInsightsAlpha(value);
                break;
            case 'insights-custom-effect':
                changed = window.state.setInsightsCustomEffectSize(value);
                break;
        }
        if (changed) {
            debouncedUpdateInsightsInput();
        }
    }
