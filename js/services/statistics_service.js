window.statisticsService = (() => {
    ...
    function calculateDeLongTest(data, key1, key2, referenceKey) {
        ...
        try {
            ...
            if (isNaN(varDiff) || varDiff <= 1e-12) {
                return { pValue: 1.0, Z: 0, diffAUC: c1.auc - c2.auc, method: "DeLong Test (Zero Variance or numerical issue)", varDiff: varDiff };
            }

            const z = (c1.auc - c2.auc) / Math.sqrt(varDiff);
            const pValue = 2.0 * normalCDF(-Math.abs(z));
            return { pValue, Z: z, diffAUC: c1.auc - c2.auc, method: "DeLong Test", varDiff: varDiff };
        } catch (error) {
    ...
    function _calculatePostHocPower(alpha, n, effectSize, varDiff) {
        if (isNaN(alpha) || isNaN(n) || isNaN(effectSize) || isNaN(varDiff) || n <= 0 || varDiff <= 0) {
            return NaN;
        }
        const z_alpha = inverseNormalCDF(1 - alpha / 2);
        const z_beta_calc = (Math.abs(effectSize) * Math.sqrt(n) / Math.sqrt(varDiff)) - z_alpha;
        return normalCDF(z_beta_calc);
    }

    function calculatePowerAndSampleSize(alpha, power, varDiff, effectSize) {
        if (isNaN(alpha) || isNaN(power) || isNaN(varDiff) || isNaN(effectSize)) {
            return { requiredSampleSize: NaN };
        }
        if (varDiff <= 0 || effectSize <= 0) return { requiredSampleSize: Infinity };

        const z_alpha = inverseNormalCDF(1 - alpha / 2);
        const z_beta = inverseNormalCDF(power);

        const requiredN = Math.pow(z_alpha + z_beta, 2) * varDiff / Math.pow(effectSize, 2);

        return {
            requiredSampleSize: Math.ceil(requiredN) || NaN
        };
    }

    function calculateMismatchAnalysis(data, t2StatusKey = 't2Status') {
        const mismatchData = {
            concordantPositive: { total: 0, N_plus: 0 },
            concordantNegative: { total: 0, N_plus: 0 },
            discordant_AS_win: { total: 0, N_plus: 0 },
            discordant_T2_win: { total: 0, N_plus: 0 }
        };

        data.forEach(p => {
            const isNplus = p.nStatus === '+';
            if (p.asStatus === '+' && p[t2StatusKey] === '+') {
                mismatchData.concordantPositive.total++;
                if (isNplus) mismatchData.concordantPositive.N_plus++;
            } else if (p.asStatus === '-' && p[t2StatusKey] === '-') {
                mismatchData.concordantNegative.total++;
                if (isNplus) mismatchData.concordantNegative.N_plus++;
            } else if (p.asStatus === '+' && p[t2StatusKey] === '-') {
                mismatchData.discordant_AS_win.total++;
                if (isNplus) mismatchData.discordant_AS_win.N_plus++;
            } else if (p.asStatus === '-' && p[t2StatusKey] === '+') {
                mismatchData.discordant_T2_win.total++;
                if (isNplus) mismatchData.discordant_T2_win.N_plus++;
            }
        });
        return mismatchData;
    }

    function calculateFeatureImportance(data, t2Criteria) {
        ...
        ['size', 'shape', 'border', 'homogeneity', 'signal'].forEach(key => {
            ...
            data.forEach(p => {
                ...
                const hasFeature = p.t2Nodes.some(lk => {
                    if (!lk) return false;
                    switch (key) {
                        case 'size': return typeof lk.size === 'number' && !isNaN(lk.size) && lk.size >= criterion.threshold;
                        case 'shape': return lk.shape === criterion.value;
                        case 'border': return lk.border === criterion.value;
                        case 'homogeneity': return lk.homogeneity === criterion.value;
                        case 'signal': return lk.signal !== null && lk.signal === criterion.value;
            ...
            if ((a + b + c + d) > 0) {
                ...
                results[key] = {
                    ...
                    featureName: `${key.charAt(0).toUpperCase() + key.slice(1)}: ${key === 'size' ? '>=' + formatNumber(criterion.threshold, 1, '', true) : criterion.value}`
                };
            }
        });
        return Object.values(results).filter(Boolean).sort((a,b) => (b.or?.value ?? 0) - (a.or?.value ?? 0));
    }
    ...
    function calculateAllPublicationStats(data, appliedT2Criteria, appliedT2Logic, allBruteForceResults) {
        ...
        cohorts.forEach(cohortId => {
            ...
            results[cohortId] = {
                ...
                insights: {
                    powerAnalysis: {},
                    mismatchAnalysis: {},
                    featureImportance: calculateFeatureImportance(cohortData, appliedT2Criteria)
                }
            };
        });
        ...
        allLiteratureSets.forEach(studySet => {
            ...
            if (dataForSet.length > 0) {
                ...
                const comparison = compareDiagnosticMethods(evaluatedDataStudy, 'asStatus', 't2Status', 'nStatus');
                results[cohortForSet].comparisonASvsT2Literature[studySet.id] = comparison;
                results[cohortForSet].addedValueAnalysis[studySet.id] = calculateAddedValue(evaluatedDataStudy);
                results[cohortForSet].insights.mismatchAnalysis[studySet.id] = calculateMismatchAnalysis(evaluatedDataStudy, 't2Status');
                if (comparison.delong && isFinite(comparison.delong.varDiff)) {
                    results[cohortForSet].insights.powerAnalysis[studySet.id] = {
                        varDiff: comparison.delong.varDiff,
                        effectSize: comparison.delong.diffAUC,
                        n: dataForSet.length,
                        postHocPower: _calculatePostHocPower(0.05, dataForSet.length, comparison.delong.diffAUC, comparison.delong.varDiff)
                    };
                }
            }
        });
            ...
            if (cohortBfResults) {
                ...
                Object.keys(cohortBfResults).forEach(metricName => {
                    ...
                    if (bfResult && bfResult.bestResult?.criteria) {
                        ...
                        const comparison = compareDiagnosticMethods(evaluatedDataBF, 'asStatus', 't2Status', 'nStatus');
                        results[cohortId].comparisonASvsT2Bruteforce[metricName] = comparison;
                        ...
                        const bfId = `bf_${cohortId}_${metricName}`;
                        results[cohortId].insights.mismatchAnalysis[bfId] = calculateMismatchAnalysis(evaluatedDataBF, 't2Status');
                        if (comparison.delong && isFinite(comparison.delong.varDiff)) {
                            results[cohortId].insights.powerAnalysis[bfId] = {
                                varDiff: comparison.delong.varDiff,
                                effectSize: comparison.delong.diffAUC,
                                n: cohortData.length,
                                postHocPower: _calculatePostHocPower(0.05, cohortData.length, comparison.delong.diffAUC, comparison.delong.varDiff)
                            };
                        }
                    }
                });
