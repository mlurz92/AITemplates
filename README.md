...
*   **Layout & Design (`style.css`)**:
    *   **`.favorites-dock`**: Vollbreites Glas-Panel am unteren Viewportrand mit Safe-Area-Puffer, sanften Ein-/Ausblendungen sowie fixen Randverläufen, die beim Scrollen nicht mitschwimmen.
    *   **`.favorites-shell` & `.favorites-toggle`**: Der Tab-Button sitzt wie ein Fähnchen auf dem Panel, zeigt den Expand-Status animiert an, respektiert Fokus-States und verschiebt beim Aufklappen nahtlos die Mehrzeilenansicht.
    *   **`.favorites-scroll-area`**: Nutzt eine horizontale Scrollspur mit dezent animierten Gradient-Overlays, reagiert auf Mausrad-Scrolling und wechselt im expandierten Zustand in einen mehrreihigen Modus.
    *   **`.favorite-chip`**: Alle Kacheln erhalten identische Breite/Höhe über CSS-Variablen. Titel und Vorschau skalieren proportional per `ResizeObserver`, Hyphenation und Soft-Truncation stellen sicher, dass Inhalte vollständig lesbar bleiben ohne zu flackern.
...
*   **Desktop (> 1024px)**: Mehrspaltiges Kartenraster (bis zu 6 Spalten). Das Favoriten-Dashboard liegt bündig am unteren Rand, füllt die Breite, verteilt gleichgroße Kacheln über die verfügbare Fläche und blendet zusätzliche Reihen via Expand-Tab ein.
*   **Tablet (768px - 1024px)**: Reduzierte Spaltenanzahl, adaptive Chip-Breiten und identische Ein-Tipp-Bedienung. Der Toggle klappt das Panel mehrreihig aus, sobald nicht alle Favoriten in einer Zeile Platz finden.
*   **Mobil (< 768px)**:
    *   Mindestens drei Karten pro Zeile im Content-Bereich.
    *   Die Favoriten-Chips skalieren automatisch in Breite und Typografie, bleiben stets vollständig lesbar und scrollen bei Bedarf horizontal mit Fix-Gradienten.
    *   Touch-Optimierungen wie Swipe-to-Go-Back und haptisches Feedback beim Kopieren bleiben aktiv.
