...
*   **HTML-Struktur (`index.html`)**:
    *   Ein `aside` mit der ID `#favorites-dock` bildet den dauerhaft sichtbaren Schnellzugriffsbereich.
    *   Innerhalb von `div.favorites-dock-inner` sitzen die Überschrift (`.favorites-dock-header`) und die ungeordnete Liste `#favorites-list`.
    *   Jede Listenzeile enthält genau einen Button der Klasse `.favorite-chip`, der sofort den verknüpften Prompt kopiert.
*   **Logik:**
...
*   **Layout & Design (`style.css`)**:
    *   **`.favorites-dock`**: Schwebendes Glas-Panel, fixiert rechts unten, respektiert Safe-Area-Insets per `--favorites-safe-offset` und blendet weich ein bzw. aus.
    *   **`.favorites-dock-inner`**: Isolation-Layer mit zweifachem Aurora-Gradient, Rausch-Overlay, Pointer-Gating und Schattenwurf für visuelle Tiefe.
    *   **`.favorites-list`**: Vertikale Spalte mit dünnem Scrollbar; auf Mobilgeräten wechselt sie automatisch in einen horizontalen Slider ohne zusätzliche Bedienung.
    *   **`.favorite-chip`**: Mehrzeilige, responsive Quick-Actions mit Line-Clamp, kontrastierendem Badge, Copy-Feedback (`.copy-success`) und sequentieller Einstiegsanimation ohne font-basiertes Resizing (verhindert Flackern).
