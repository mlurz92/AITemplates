...
*   **Design-Philosophie:** Ein modernes **"Glassmorphism"-UI** mit animierten, leicht angeleuchteten Rauchschleiern als Hintergrund. Transparente, schwebende Ebenen mit `backdrop-filter`, der leichte Humanist-Font **Manrope** sowie subtil abgestimmte Licht- und Schatten-Effekte schaffen eine hochwertige visuelle Tiefe.
*   **Layout-Schutz für Karten:** Ein JavaScript-gestützter Layout-Manager ermittelt in Echtzeit die verfügbare Containerbreite, erzwingt mindestens drei und höchstens sechs Spalten und skaliert Karten, Typografie sowie Innenabstände synchron per CSS-Variablen. Prompt- und Ordnerkarten bleiben dadurch in ihrem ursprünglichen Größenverhältnis **1 : 0,8** (Breite zu Höhe) erhalten; Ordnerkarten behalten zusätzlich ihren 0,8‑Faktor zur Prompt-Höhe. Selbst in extrem schmalen Viewports bleibt die Beschriftung komplett lesbar, ohne dass Nachbarn überlagert werden.
...
*   **Layout (`style.css`)**:
    ...
    *   `aspect-ratio: calc(var(--card-base-aspect) / var(--card-height-ratio))` sorgt dafür, dass Prompt-Karten stets im Verhältnis 1 : 0,8 und Ordner-Karten im Verhältnis 1 : 0,64 (auf Basis des Prompt-Werts) bleiben – unabhängig von der aktuellen Breite.
...
*   **Liquid-Smog-Hintergrund**: Drei großflächige, stark weichgezeichnete Ebenen mit radialen Farbverläufen in Türkis-, Mint- und Flieder-Tönen schweben auf dem gesamten Viewport. Per CSS-Keyframes ziehen sie als subtil angeleuchtete Rauchschwaden vorbei, wodurch der Glass-Look Tiefe erhält, ohne den Inhalt zu überstrahlen.
*   **Parallax-Effekt**: Beim Scrollen der Karten wird der Nebel-Container per `requestAnimationFrame` gedrosselt und mit einem geringeren Faktor verschoben (`transform: translateY(...)`), was einen subtilen Tiefeneffekt erzeugt.
