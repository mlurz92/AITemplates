...
const FAVORITE_CHIP_MIN_WIDTH = 200;
const FAVORITE_CHIP_MAX_WIDTH = 460;
const FAVORITE_CHIP_MIN_WIDTH_NARROW = 160;
const FAVORITE_FULL_LAYOUT_THRESHOLD = 340;
const FAVORITE_COMPACT_LAYOUT_THRESHOLD = 260;
const FAVORITE_TITLE_MIN_SCALE = 0.72;
...
function applyFavoriteChipMetrics() {
    ...
    let widthCandidate = (availableWidth - gap * (columns - 1)) / columns;
    const needsOverflow = count > columns;
    const widthCap = needsOverflow
        ? FAVORITE_CHIP_MAX_WIDTH
        : Math.min(Math.max(FAVORITE_CHIP_MAX_WIDTH, widthCandidate), 640);
    const lowestAllowed = Math.max(FAVORITE_CHIP_MIN_WIDTH_NARROW, Math.min(FAVORITE_CHIP_MIN_WIDTH, availableWidth));
    let targetWidth = Math.max(lowestAllowed, Math.min(widthCap, widthCandidate));

    if (!expanded && count === 1) {
        const minSoloWidth = Math.min(FAVORITE_CHIP_MIN_WIDTH_NARROW, availableWidth);
        targetWidth = Math.max(minSoloWidth, Math.min(targetWidth, availableWidth));
    }

    const baseHeight = Math.max(100, Math.min(148, targetWidth * 0.52));

    favoritesDockEl.style.setProperty('--favorite-chip-width', `${targetWidth}px`);
...
function applyFavoriteChipContent(chip, layout, width) {
    ...
    const maxChars = layout === 'full'
        ? Math.max(120, Math.floor(width * 0.95))
        : Math.max(60, Math.floor(width * 0.7));

    previewEl.textContent = trimPreviewForLayout(fullPreview, maxChars);
